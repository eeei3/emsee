...
function check_input_details(event_details : Map<string,string>) {
    ...
    if ((event_details.get("id") == undefined) || !check_event_id(Number(event_details.get("id")))) {
	return false;
    }
    let match = 0;
    const event_entries = ["id", "event_name", "desc", "loc", "event_time", "cost", "max_att", "curr_att"]
...
export async function update_event_details(event_details: Map<string, string>) {
    ...
    const event_time = new Date(String(event_details.get("event_time")));
    if (!isNaN(event_id) && !isNaN(cost) && !isNaN(max_att) && !isNaN(curr_att) && event_time.toString() != "Invalid Date") {
	try {
	    const updatedId: { target_id: number }[] = await db.update(events).set({
		name:event_details.get("event_name"), 
		description: event_details.get("desc"),
		location: event_details.get("loc"),
	    ...
	    if (updatedId.length == 0 || isNaN(updatedId[0].target_id)) {
		return -3;
	    }
	    return 0;
	}
