...
export async function POST(
  ...
  try {
    // const { event_id, raw_event_details } = await params
    const { event_id } = await params;

    let parsed_event_details = null;
    let raw_event_details = ""; 

    // I think the issue is with scope, raw_event_details gets set to "" in the next try-catch
    try {
	try {
	    raw_event_details = await request.text();
	    parsed_event_details = JSON.parse(raw_event_details);
	    parsed_event_details["id"] = event_id;
	    console.log(parsed_event_details);
	    console.log(typeof parsed_event_details);
	}
	catch {
